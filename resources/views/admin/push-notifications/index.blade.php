<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div class="flex items-center gap-2">
                <span class="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1.5 text-xs font-bold text-zinc-300">
                    {{ $activeDevices }} dispositivos activos
                </span>
                <span class="rounded-full px-3 py-1.5 text-xs font-black {{ $configured ? 'bg-emerald-500/10 text-emerald-300' : 'bg-amber-500/10 text-amber-300' }}">
                    {{ $configured ? 'Firebase conectado' : 'Firebase pendiente' }}
                </span>
            </div>
        </div>
    </x-slot>
    ...
    <div class="admin-container py-7 space-y-7">
        ...
        @endif
        @if(session('warning'))
            <div class="rounded-2xl border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">{{ session('warning') }}</div>
        @endif
        @if(session('error'))
            <div class="rounded-2xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-200">{{ session('error') }}</div>
        @endif
        ...
        <section class="grid gap-6 xl:grid-cols-[1.05fr_.95fr]">
            <div class="admin-panel p-6">
                <div class="mb-6 flex items-center gap-3">
                    ...
                    <div>
                        <h2 class="text-lg font-black text-white">Nueva notificación</h2>
                        <p class="text-sm text-zinc-500">Se enviará a todos los dispositivos activos.</p>
                    </div>
                </div>
                ...
                <form method="POST" action="{{ route('admin.push-notifications.store') }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <input id="title" name="title" value="{{ old('title') }}" maxlength="120" required
                               placeholder="Ej. Estamos al aire"
                               class="w-full rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 text-white placeholder:text-zinc-600 focus:border-orange-500 focus:ring-orange-500">
                    ...
                    <div>
                        ...
                        <textarea id="body" name="body" rows="5" maxlength="500" required
                                  placeholder="Ej. Somos Radio 89.1 FM está en vivo. Escúchanos ahora."
                                  class="w-full rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 text-white placeholder:text-zinc-600 focus:border-orange-500 focus:ring-orange-500">{{ old('body') }}</textarea>
                    ...
                    <div class="rounded-2xl border border-orange-500/15 bg-orange-500/[0.06] p-4 text-sm text-zinc-400">
                        <span class="font-bold text-orange-300">Vista previa:</span>
                        la notificación aparecerá con el nombre e icono de la app en el teléfono del oyente.
                    </div>

                    <button type="submit" class="admin-btn-primary w-full justify-center" {{ !$configured || $activeDevices === 0 ? 'disabled' : '' }}>
                        Enviar notificación
                    </button>

                    @if(!$configured)
                        <p class="text-center text-xs text-amber-300">Configura las credenciales de Firebase en el servidor para habilitar el envío.</p>
                    @elseif($activeDevices === 0)
                        <p class="text-center text-xs text-zinc-500">Aún no hay dispositivos registrados desde la app.</p>
                    @endif
                </form>
            </div>
            ...
            <div class="admin-panel overflow-hidden">
                <div class="border-b border-white/10 px-6 py-5">
                    ...
                    <h2 class="mt-1 text-lg font-black text-white">Últimos envíos</h2>
                </div>
                    ...
                    @forelse($notifications as $notification)
                        <div class="px-6 py-5">
                            <div class="flex items-start justify-between gap-4">
                                <div class="min-w-0">
                                    <div class="truncate font-black text-white">{{ $notification->title }}</div>
                                    <p class="mt-1 line-clamp-2 text-sm text-zinc-500">{{ $notification->body }}</p>
                                </div>
                                @php
