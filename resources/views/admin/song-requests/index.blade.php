        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
                    <div class="text-xs uppercase tracking-wide text-gray-500">Nuevas</div>
                    <div class="text-3xl font-bold text-gray-900 mt-1">{{ $counts['new'] }}</div>
                </div>
                <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
                    <div class="text-xs uppercase tracking-wide text-gray-500">Vistas</div>
                    <div class="text-3xl font-bold text-gray-900 mt-1">{{ $counts['seen'] }}</div>
                </div>
                <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
                    <div class="text-xs uppercase tracking-wide text-gray-500">Atendidas</div>
                    <div class="text-3xl font-bold text-gray-900 mt-1">{{ $counts['attended'] }}</div>
                </div>
                <div class="bg-white rounded-xl shadow-sm p-4 border border-gray-100">
                    <div class="text-xs uppercase tracking-wide text-gray-500">Descartadas</div>
                    <div class="text-3xl font-bold text-gray-900 mt-1">{{ $counts['discarded'] }}</div>
                </div>
            </div>
            ...
            <form method="GET" class="bg-white rounded-xl shadow-sm p-4 border border-gray-100 flex flex-col md:flex-row gap-3 md:items-end">
                ...
                <div class="flex gap-2">
                    <button type="submit" class="px-4 py-2 rounded-md bg-gray-900 text-white font-medium hover:bg-black transition">Filtrar</button>
                                ...
                                <div class="min-w-0">
                                    <div class="flex flex-wrap items-center gap-2 mb-2">
                                        <span class="inline-flex items-center rounded-full bg-orange-100 px-2.5 py-1 text-xs font-semibold text-orange-700">
                                            {{ $songRequest->channel?->name ?? 'Estación' }}
                                        </span>
                                        @php
                                            $statusClasses = [
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold {{ $statusClasses[$songRequest->status] ?? 'bg-gray-100 text-gray-600' }}">
                                            {{ $statusLabels[$songRequest->status] ?? $songRequest->status }}
                                        </span>
                                    ...
                                    <h3 class="text-xl font-semibold text-gray-900">
                                        {{ $songRequest->song }}
                                        <span class="font-normal text-gray-500">— {{ $songRequest->artist }}</span>
                                    </h3>

                                    <p class="text-sm text-gray-600 mt-2">
                                        Solicitada por <span class="font-semibold text-gray-800">{{ $songRequest->listener_name }}</span>
                                    </p>

                                    @if($songRequest->dedication)
                                        <div class="mt-3 rounded-lg bg-gray-50 px-4 py-3 text-sm text-gray-700 border border-gray-100">
                                            “{{ $songRequest->dedication }}”
                                        </div>
                                    @endif
                                </div>

                                <form action="{{ route('admin.song-requests.update', $songRequest) }}" method="POST" class="flex items-center gap-2 shrink-0">
                                    @csrf
                                    @method('PATCH')
                                    <select name="status" class="rounded-md border-gray-300 shadow-sm text-sm focus:border-orange-500 focus:ring-orange-500">
                                    ...
                                    <button type="submit" class="px-3 py-2 rounded-md bg-orange-500 text-white text-sm font-semibold hover:bg-orange-600 transition">
                                        Guardar
                                    </button>
                                </form>
                            </div>
